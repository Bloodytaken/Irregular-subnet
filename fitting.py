...
def compute_NMSE(*A) -> torch.Tensor:
    ...
    Assumes input A is (model, upast, ypast, ufuture, yfuture, delta_t_past, delta_t_future)'''
    # A = (model, upast, ypast, ufuture, yfuture, delta_t_past, delta_t_future)
    model          = A[0]
    upast          = A[1]
    ypast          = A[2]
    ufuture        = A[3]
    yfuture_target = A[4] # <- target y
    delta_t_past   = A[5]
    delta_t_future = A[6]
    yout = model(upast, ypast, ufuture, yfuture_target, delta_t_past, delta_t_future)
    variance = model.norm.ystd**2 + 1e-10
    return torch.mean((yout - yfuture_target)**2 / (variance))
...
def fit(model: nn.Module, train:Input_output_data, val:Input_output_data, n_its:int, T:int=50, \
    ...
    # arrays_val, indices = model.create_arrays(val, T=T, stride=stride)
    ...
    it_counter = d['it_counter']
    NRMS_train = d['NRMS_train']
    NRMS_val = d['NRMS_val']

    plt.figure(figsize=(5,3))
    plt.plot(it_counter, NRMS_train, label='Train Loss (NRMS)')
    plt.plot(it_counter, NRMS_val, label='Validation Loss (NRMS)')
    plt.xlabel('Iterations')
    plt.ylabel('NRMS Loss')
    plt.title('Training & Validation NRMS Loss over Iterations')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
